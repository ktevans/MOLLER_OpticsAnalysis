...
//------------------------------------------------------------------------------
  ...
  auto pred_tgraph = df_with_resid.Graph("gem1_ph","tg_ph_pred");
  ...
  auto true_tgraph = df_with_resid.Graph("gem1_ph","tg_ph");
  ...
  auto residuals_tgraph = df_with_resid.Graph("gem1_ph","residuals_ph");
...
//------------------------------------------------------------------------------
  ...
  multigraphC -> SetTitle("True (red) and predected (blue) values for target phi vs GEM1 phi");
  ...
  graphC->SaveAs("opticsAnalysis/opticsMatrix/ResidualPlots/phi/tg_ph_vs_gem1_ph_"+trgt+"_"+pass+"_"+hole_id+".pdf");
  ...
  gPad -> Modified();
  gPad -> Update();
  resid_can -> SaveAs("opticsAnalysis/opticsMatrix/ResidualPlots/phi/residuals_ph_vs_gem1_ph_"+trgt+"_"+pass+"_"+hole_id+".pdf");
