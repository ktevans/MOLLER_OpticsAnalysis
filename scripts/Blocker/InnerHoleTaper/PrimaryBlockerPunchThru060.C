...
//   build/reroot
//   .L scripts/BlockerPunchThru060.C
//   BlockerPunchThru060()
//
...
#include <TF1.h>
void PrimaryBlockerPunchThru060()
{
  //Allow Tree to use the root files that you call
  TChain* T = new TChain("T");

  //Add files from the directory "rootfilesPT/", "060" is the Blocker thickness in mm. Replace these root files with whatever root files you would like to analyze.
...
//----------------------------------------------------------------------------
  ...
  TH2F *us_Blocker_virt = new TH2F("us_Blocker_virt","Upstream Blocker (060mm) Virtual Detector Charged Particle Hits", 100, -150, 150, 100, -150, 150);
  ...
  TH2F *ds_Blocker_virt = new TH2F("ds_Blocker_virt","Downstream Blocker (060mm) Virtual Detector Charged Particle Hits", 100, -150, 150, 100, -150, 150);
  ...
  TH1F *radial_main = new TH1F("radial_main","Energies of Charged Particles that Exited the Downstream Blocker (060mm) Surface and Hit the Main Detector", 50, 0, 1500);
  ...
  TH2F *punch_thru_xy_main = new TH2F("punch_thru_xy_main","Charged Particles that Exited the Downstream Blocker (060mm) Surface and Hit the Main Detector", 100, -1200, 1200, 100, -1200, 1200);
  ...
  TH1F *radial_main_all = new TH1F("radial_main_all","Energies of All Charged Particles that Hit the Main Detector for a 060mm Blocker", 50, 0, 1500);
  ...
  TH2F *xy_main_all = new TH2F("xy_main_all","All Charged Particles that Hit the Main Detector for a 060mm Blocker", 100, -1200, 1200, 100, -1200, 1200);
...
//----------------------------------------------------------------------------
  ...
  for (size_t iev = 0; iev < T->GetEntries(); iev++)
  {
    ...
    for (size_t ihit = 0; ihit < hits->size(); ihit++)
    {
      ...
      if ((hit.trid==1) && hit.r<97.9932)
      {

          if (hit.det == 1004 && hit.pz >= 0 && hit.r >= 31.40)
          {
            us_Blocker_virt -> Fill(hit.x, hit.y, rate);
          }//end upstream if

          if (hit.det == 1005 && hit.pz >= 0 && hit.r >= 35)
          {
    ...
    for (size_t ihit = 0; ihit < hits->size(); ihit++)
    {
      remollGenericDetectorHit_t hit = hits->at(ihit);
        ...
        if (punch_thru && hit.det==28 && hit.r>=650 && hit.r<=1200 && (hit.trid==1) && hit.pz>=0)
        {
        ...
        if (hit.det==28 && hit.r>=650 && hit.r<=1200 && hit.pz>=0 && (hit.trid==1))
        {
...
//Draw and save the histograms
  ...
  virt -> SaveAs("images/Blocker/Taper/Primary_Punch_Thru_US_DS_WBlocker_060.png");
  ...
  //Save the canvas as an image in the director "remoll/images/Blocker/"
  main -> SaveAs("images/Blocker/Taper/Primary_Punch_Thru_Main_WBlocker_060.png");
  ...
  //Save the canvas as an image in the director "remoll/images/Blocker/"
  main_all_charged -> SaveAs("images/Blocker/Taper/Primary_Main_All_WBlocker_060.png");
