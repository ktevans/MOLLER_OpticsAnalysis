...
void NewDetailedSieve04HoleAnalysisRing2x()
{
...
//---Define Histograms---
  ...
  TH1F *radial_clean_pass = new TH1F("radial_clean_pass","Charged Particles that Pass Through Any of the Sieve (100mm) Holes (04mm Diameter) and Hit the Main Detector", 1000, 710, 760);
  ...
  TH1F *radial_hole_shower = new TH1F("radial_hole_shower","Charged Particles that Result from Primaries Hitting a Sieve Hole (04mm Diameter) and then Showering Through Sieve (100mm) and Hit the Main Detector", 1000, 710, 760);
  ...
  TH1F *radial_hit_shower = new TH1F("radial_hit_shower","Charged Particles that Result from Primaries Hitting the Sieve Surface and then Showering into a Sieve (100mm) Hole (04mm Diameter) and Hit the Main Detector", 1000, 710, 760);
...
//----------------------------------------------------------------------------
  ...
  for (size_t iev = 0; iev < T->GetEntries(); iev++)
  {
    ...
    for (size_t ihit = 0; ihit < hits->size(); ihit++)
    {
      ...
      if((hit.pid == 11 || hit.pid == -11) && (hit.det == 1001 || hit.det == 1002 || hit.det == 1003) && abs(hit.z - 575) < 0.1 && hit.pz >= 0)
      {
        in_hole = true;
      }
...
for (size_t iev = 0; iev < T->GetEntries(); iev++)
{
    ...
    //Process hits because we will now be filling histograms.
    for (size_t ihit = 0; ihit < hits->size(); ihit++)
    {
...
//---Separate the Histograms---
  ...
  separate -> SaveAs("images/HA/Only_CP_04Hole_Analysis_Sieve_100_Ring2.png");
