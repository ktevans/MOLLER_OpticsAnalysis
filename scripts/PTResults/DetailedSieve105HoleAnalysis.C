...
//   build/reroot
//   .L scripts/DetailedSieve105HoleAnalysis1.C
//   DetailedSieve105HoleAnalysis()
//  Or use:
//   build/reroot -l -q -b 'DetailedSieve105HoleAnalysis1.C()'
//
...
//105 is the sieve thickness in mm
void DetailedSieve105HoleAnalysis()
{
...
//---Define Histograms---
  ...
  TH1F *radial_clean_pass = new TH1F("radial_clean_pass","Charged Particles that Cleanly Pass Through Any of the Sieve (105mm) Holes and Hit the Main Detector", 1000, 650, 1200);
  ...
  TH1F *radial_hole_shower = new TH1F("radial_hole_shower","Charged Particles that Result from Primaries Hitting a Sieve Hole and then Showering Through Sieve (105mm) and Hit the Main Detector", 1000, 650, 1200);
  ...
  TH1F *radial_hit_shower = new TH1F("radial_hit_shower","Charged Particles that Result from Primaries Hitting the Sieve Surface and then Showering into a Sieve (105mm) Hole and Hit the Main Detector", 1000, 650, 1200);
...
//----------------------------------------------------------------------------
  ...
  for (size_t iev = 0; iev < T->GetEntries(); iev++)
  {
    ...
    for (size_t ihit = 0; ihit < hits->size(); ihit++)
    {
      ...
      if((hit.trid == 1 || hit.trid == 2 || hit.pid == 11 || hit.pid == -11) && (hit.det == 1001 || hit.det == 1002 || hit.det == 1003) && abs(hit.z - 572.5) < 0.1 && hit.pz >= 0)
      {
        in_hole = true;
      }
...
//---Draw and Save Histograms---
  ...
  auto titleCleanPass = new TPaveLabel(0.0, 0.9, 1.0, 1.0, "Primaries that Hit the Main Detector Rings for a 105mm Full Tungsten Sieve", "brNDC");
  titleCleanPass -> Draw();
  ...
  cleanpass -> SaveAs("images/PTResults/CP_Hole_Analysis_Sieve_105.png");
  ...
  separate -> SaveAs("images/PTResults/CP_Hole_Analysis_Sieve_105_Separate.png");

}//end DetailedSieve105HoleAnalysis()
