...
void Blocker(const TString& Thickness, const double t)
{
  ...
  //----------------------------------------------------------------------------
    ...
    TH2F *US_surface = new TH2F("US_surface","All Charged Particles that Hit the Upstream Surface of a "+Thickness+" mm Tapered Blocker", 100, -120, 120, 100, -120, 120);
    ...
    TH2F *DS_surface = new TH2F("DS_surface","All Charged Particles that Hit the Upstream Surface of a "+Thickness+" mm Tapered Blocker", 100, -120, 120, 100, -120, 120);
    ...
    for (size_t iev = 0; iev < T->GetEntries(); iev++)
    {
      ...
      for (size_t ihit = 0; ihit < hits->size(); ihit++)
      {
        remollGenericDetectorHit_t hit = hits->at(ihit);
          ...
          if (hit.pz>=0 && (hit.pid==11 || hit.pid==-11))
          {
            if (hit.det==1004 && hit.r<97.9932 && hit.r>=29)
            {
              US_surface -> Fill(hit.x, hit.y, rate);
            }

            if (hit.det==1005 && hit.r<97.9932 && hit.r>=r)
            {
...
virt -> SaveAs("images/Blocker/InnerHoleTaper/CP_US_DS_TWBlocker_"+Thickness+".png");
