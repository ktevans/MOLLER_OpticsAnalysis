...
//  Need to have a subdirectory "images/Blocker" defined for the images to go
//  to.
//
//  Run using (using a thickness of 100mm for example):
//   build/reroot
//   .L scripts/BlockerCPAnalysis.C
//   Blocker("100")
//
...
//
//  This script will take root files of a certain Blocker thickness.
//  All charged particles that reach the main detector rings are plotted (xy and
...
void Blocker(const TString& Thickness)
{
  ...
  T->Add("rootfilesPT/blocker/35TWBlockerC12_ep_100k_"+Thickness+"mm0.root");
  T->Add("rootfilesPT/blocker/35TWBlockerC12_ep_100k_"+Thickness+"mm1.root");
  T->Add("rootfilesPT/blocker/35TWBlockerC12_ep_100k_"+Thickness+"mm2.root");
  T->Add("rootfilesPT/blocker/35TWBlockerC12_ep_100k_"+Thickness+"mm3.root");
  T->Add("rootfilesPT/blocker/35TWBlockerC12_ep_100k_"+Thickness+"mm4.root");
  T->Add("rootfilesPT/blocker/35TWBlockerC12_ep_100k_"+Thickness+"mm5.root");
  T->Add("rootfilesPT/blocker/35TWBlockerC12_ep_100k_"+Thickness+"mm6.root");
  T->Add("rootfilesPT/blocker/35TWBlockerC12_ep_100k_"+Thickness+"mm7.root");
  T->Add("rootfilesPT/blocker/35TWBlockerC12_ep_100k_"+Thickness+"mm8.root");
  T->Add("rootfilesPT/blocker/35TWBlockerC12_ep_100k_"+Thickness+"mm9.root");
...
//----------------------------------------------------------------------------
  ...
  TH1F *ring2_all = new TH1F("ring2_all","All Charged Particles that Hit Ring 2 on the Main Detector for a "+Thickness+" mm Blocker", 1000, 710, 760);
  ring2_all -> GetXaxis() -> SetTitle("r Position of Hits [mm]");

  //1D histogram for particles that make it to the main detector on the ring 2 (phi plot)
  TH1F *ring2_all_ph = new TH1F("ring2_all_ph","All Charged Particles that Hit Ring 2 on the Main Detector for a "+Thickness+" mm Blocker", 1000, -4, 4);
  ring2_all_ph -> GetXaxis() -> SetTitle("Azimuthal Angle of Hits [mrad]");

  //1D histogram for z vertices of particles that make it to the main detector on the rings (z plot)
  TH1F *vertex_main = new TH1F("vertex_main","Z Vertex of Charged Particles that Hit the Main Detector for a "+Thickness+" mm Blocker", 100, -4500, 22000);
  ...
  TH2F *xy_main_all = new TH2F("xy_main_all","All Charged Particles that Hit the Main Detector for a "+Thickness+" mm Blocker", 100, -1200, 1200, 100, -1200, 1200);
    ...
    for (size_t ihit = 0; ihit < hits->size(); ihit++)
    {
      remollGenericDetectorHit_t hit = hits->at(ihit);
        ...
        if (hit.det==28 && hit.r>=650 && hit.r<=1200 && hit.pz>=0 && (hit.pid==11 || hit.pid==-11))
        {
          ...
          {
            ring2_all -> Fill(hit.r);
            ring2_all_ph -> Fill(hit.ph, rate);
          }

          if ((hit.r>=720 && hit.r<=745) && (hit.ph>=2.1 && hit.ph<=2.4))
          {
...
//Draw and save the histograms
  ...
  main_all_vr -> SaveAs("images/CP_Main_All_35WBlocker_"+Thickness+".png");

  //---Main Detector Vertex and Ring 2---

  //Create a canvas on which to draw the histograms
  TCanvas *main_vr = new TCanvas("main_vr","Particles Hitting the Main Detector", 1000, 1000, 1000, 1000);
  ...
  main_vr -> SaveAs("images/CP_Main_35mm_vertex_WBlocker_"+Thickness+".png");
