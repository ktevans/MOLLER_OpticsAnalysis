...
const int nFiles = 5;
const int nPlots = 24;
const char* filename[nFiles] = { "slim_output/Cfoil_optics1DS_p1.root", "slim_output/Cfoil_optics1DS_p2.root","slim_output/Cfoil_optics1DS_p3.root", "slim_output/Cfoil_optics1DS_p4.root","slim_output/Cfoil_optics1DS_p5.root"};
...
const char* plotName[nPlots] = { "main_xy","sieve_xy","gem1_xy","gem2_xy","gem3_xy","gem4_xy","gem1_r","gem1_r'","gem1_phi","gem1_phi'",
				 "theta_tg_vs_r_gem1","theta_tg_vs_phi_gem1","theta_tg_vs_r'_gem1","theta_tg_vs_phi'_gem1" ,
                                 ...
                                 "tg_p_vs_r_gem1","tg_p_vs_phi_gem1","tg_p_vs_r'_gem1","tg_p_vs_phi'_gem1" , "sieve_r", "sieve_phi"};
...
TH2D* Etheta_weighted[nFiles] = {NULL};  // E' vs theta_tg weighted with rate
...
void plots(){
  ...
  for(int i=0; i<nFiles; i++){
    ...
    double main_x, main_y, sieve_x, sieve_y, sieve_r, sieve_ph;
    ...
    sievephi[i] = new TH1D(Form("sieve_phi_%s",passNames[i]),"sieve_phi_distribution;#phi(rad);counts",100,-pi,pi);
    ...
    phir[i] = new TH2D(Form("phi_r_gem1_%s",passNames[i]),"phi_r_gem1_distribution;#phi_{tg};r[mm]", 100, -pi, pi,100, 500, 1300);
    phirp[i] = new TH2D(Form("phi_r'_gem1_%s",passNames[i]),"phi_r_prime_gem1_distribution;#phi_{tg};r'", 100,-pi, pi,100, 0, 0.1);
    phiphi[i] = new TH2D(Form("phi_phi_gem1_%s",passNames[i]),"phi_phi_gem1_distribution;#phi_{tg};#phi",100, -pi, pi, 100, -pi, pi);
    phiphip[i] = new TH2D(Form("phi_phi'_gem1_%s",passNames[i]),"phi_phi_prime_gem1_distribution;#phi_{tg};#phi'",100, -pi, pi,1000, -0.01, 0.01);
    ...
    pphi[i] = new TH2D(Form("tg_p_phi_gem1_%s",passNames[i]),"tg_p_phi_gem1_distribution;#phi;E'(GeV)", 100, -pi, pi, 100, 0, 12);
    ...
    for(int j=0; j<nEntries; j++){  
      ...
      double phi_sieve = atan2(sieve_y,sieve_x);
      ...
      thetar[i]->Fill(gem1_r,tg_th*(180/pi),rate); thetaphi[i]->Fill(gem1_phi,tg_th*(180/pi),rate);
  ...
  for(int i=0; i<nPlots; i++){
    ...
    if(i==0) c[i]->Print("initial_plots.pdf("); 
    if(i==nPlots-1) c[i]->Print("initial_plots.pdf)");
    else c[i]->Print("initial_plots.pdf");
