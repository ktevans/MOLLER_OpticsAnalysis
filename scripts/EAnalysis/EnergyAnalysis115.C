...
//----------------------------------------------------------------------------
  ...
  TH2F *us_sieve_virt_energy = new TH2F("us_sieve_virt_energy","Upstream Sieve (115mm) Virtual Detector Primary Energies for ep Scattering with Thin Carbon Target", 7000, 0, 1500, 7000, 30, 110);
  ...
  TH2F *ds_sieve_virt_energy = new TH2F("ds_sieve_virt_energy","Downstream Sieve (115mm) Virtual Detector Primary Energies for ep Scattering with Thin Carbon Target", 7000, 0, 1500, 7000, 30, 110);
  ds_sieve_virt_energy -> GetXaxis() -> SetTitle("Energy of Hits [MeV]");
  ds_sieve_virt_energy -> GetYaxis() -> SetTitle("r Position of Hits [mm]");

  TH1F *prim_clean_pass = new TH1F("prim_clean_pass","Primaries that Pass Through the Sieve (115mm) Holes without Losing Energy and Hit the Main Detector Rings", 1000, 650, 1200);
  prim_clean_pass -> GetXaxis() -> SetTitle("r Position of Hits [mm]");

  TH1F *prim_pass = new TH1F("prim_pass","All Primaries that Hit the Main Detector Rings with a W Sieve (115mm)", 1000, 650, 1200);
...
//Draw and save the histograms
  ...
  virt -> SaveAs("images/EAnalysis/US_DS_Prim_Energy_WSieve_115.png");
  ...
  main_all -> SaveAs("images/EAnalysis/same_energy_main_WSieve_115.png");

  TCanvas *main_all_prim = new TCanvas("main_all_prim","Particles Hitting the Main Detector", 1000, 1000, 1000, 1000);

  prim_pass -> Draw();

  main_all_prim -> SaveAs("images/EAnalysis/prim_main_WSieve_115.png");
