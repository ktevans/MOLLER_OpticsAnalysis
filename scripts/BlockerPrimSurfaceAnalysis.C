...
void Blocker(const TString& Thickness, const double t)
{
  ...
  //----------------------------------------------------------------------------
    ...
    TH2F *US_surface = new TH2F("US_surface","All Primaries that Hit the Upstream Surface of a "+Thickness+" mm Blocker", 100, -120, 120, 100, -120, 120);
    ...
    TH2F *DS_surface = new TH2F("DS_surface","All Primaries that Hit the Upstream Surface of a "+Thickness+" mm Blocker", 100, -120, 120, 100, -120, 120);
    ...
    for (size_t iev = 0; iev < T->GetEntries(); iev++)
    {
      ...
      for (size_t ihit = 0; ihit < hits->size(); ihit++)
      {
        remollGenericDetectorHit_t hit = hits->at(ihit);
          ...
          if (hit.pz>=0 && hit.trid==1 && hit.r<97.9932 && hit.r>=r)
          {
            if (hit.det==1004)
            {
...
virt -> SaveAs("images/Blocker/InnerHoleTaper/Prim_US_DS_WBlocker_"+Thickness+".png");
