...
//------------------------------------------------------------------------------
  ...
  auto pred_tgraph = df_with_resid.Graph("gem1_php","tg_th_pred");
  ...
  auto true_tgraph = df_with_resid.Graph("gem1_php","tg_th");
  ...
  auto residuals_tgraph = df_with_resid.Graph("gem1_php","residuals_th");
...
//------------------------------------------------------------------------------
  ...
  multigraphC -> SetTitle("True (red) and predected (blue) values for target theta vs GEM1 phi prime");
  //Changed title, so update gPad on which canvas is made.
  gPad -> Modified();
  gPad -> Update();
  //Set x and y axis titles.
  multigraphC -> GetXaxis() -> SetTitle("Phi' value of hits on GEM1 plane [radians/s]");
  ...
  graphC->SaveAs("opticsAnalysis/opticsMatrix/ResidualPlots/theta/tg_th_vs_gem1_php_"+trgt+"_"+pass+"_"+hole_id+".pdf");
  ...
  gPad -> Modified();
  gPad -> Update();
  resid_multi -> GetXaxis() -> SetTitle("Phi' value of hits on GEM1 plane [radians/s]");
  ...
  gPad -> Modified();
  gPad -> Update();
  resid_can -> SaveAs("opticsAnalysis/opticsMatrix/ResidualPlots/theta/residuals_th_vs_gem1_php_"+trgt+"_"+pass+"_"+hole_id+".pdf");
