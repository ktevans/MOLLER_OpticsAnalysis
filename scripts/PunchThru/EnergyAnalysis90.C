...
//----------------------------------------------------------------------------
  ...
  TH2F *us_sieve_virt_energy = new TH2F("us_sieve_virt_energy","Upstream Sieve (90mm) Virtual Detector Electron and Positron Energies for ep Scattering with Thin Carbon Target", 7000, -150, 150, 7000, -150, 150);
  ...
  TH2F *ds_sieve_virt_energy = new TH2F("ds_sieve_virt_energy","Downstream Sieve (90mm) Virtual Detector Electron and Positron Energies for ep Scattering with Thin Carbon Target", 7000, -150, 150, 7000, -150, 150);
  ds_sieve_virt_energy -> GetXaxis() -> SetTitle("Energy of Hits [MeV]");
  ds_sieve_virt_energy -> GetYaxis() -> SetTitle("r Position of Hits [mm]");

  TH1F *prim_clean_pass = new TH1F("prim_clean_pass","Electrons and Positrons that Pass Through the Sieve (90mm) Holes without Losing Energy and Hit the Main Detector Rings", 1000, 650, 1200);
  ...
  for (size_t iev = 0; iev < T->GetEntries(); iev++)
  {
    ...
    for (size_t ihit = 0; ihit < hits->size(); ihit++)
    {
      ...
      if ((hit.pid==11 || hit.pid==-11) && hit.r >=35.306 && hit.r<97.9932)
      {
          ...
          if(abs(in_e - out_e) <= 1 && (hit.trid == 1 || hit.trid == 2) && hit.det ==28 && hit.r >= 650 && hit.r <= 1200 && hit.pz >= 0)
          {
