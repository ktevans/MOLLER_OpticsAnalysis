...
//residual_plots(0.29488038, 0.0, -0.000987003294, 4.18617906, 0.0013268247, 0.887124893, 0.00000105555021, -0.0122292953, -0.00000273453983, -0.00224508165, 45.4856069, 0.015641087, 18.6460331, -0.00000659396964, -0.0277582572, -29.1404041, "1DS", "p3", "11", "opticsAnalysis/opticsMatrix/dataFiles/output/optics1DS/optics1DS_p3_11.csv")
...
//------------------------------------------------------------------------------
  ...
  auto pred_tgraph = df_with_resid.Graph("gem1_r","tg_ph_pred");
  ...
  auto true_tgraph = df_with_resid.Graph("gem1_r","tg_ph");
  ...
  auto residuals_tgraph = df_with_resid.Graph("gem1_r","residuals_ph");
  ...
  //Plot a simple 1D plot of the residual values.

  auto oned_residuals_tgraph = df_with_resid.Histo1D("residuals_ph");
  auto oned_residuals_tgraph_ptr = oned_residuals_tgraph.GetPtr();
...
//------------------------------------------------------------------------------
  ...
  multigraphC -> SetTitle("True (red) and predected (blue) values for target phi vs GEM1 radius");
  ...
  graphC->SaveAs("opticsAnalysis/opticsMatrix/ResidualPlots/phi/tg_ph_vs_gem1_r_"+trgt+"_"+pass+"_"+hole_id+".pdf");
  ...
  gPad -> Modified();
  gPad -> Update();
  resid_can -> SaveAs("opticsAnalysis/opticsMatrix/ResidualPlots/phi/residuals_ph_vs_gem1_r_"+trgt+"_"+pass+"_"+hole_id+".pdf");


  //Plot Histogram

  auto canvas_oned_resid = new TCanvas();
  gStyle -> SetOptFit(1111);
  oned_residuals_tgraph_ptr -> DrawCopy();
  oned_residuals_tgraph_ptr -> Fit("gaus","Q");
  gPad -> Modified();
  gPad -> Update();
  TPaveLabel *Plab1 = new TPaveLabel(0.2, 0.95, 0.8, 1.0, "Residuals for Target Phi", "NDC");
  Plab1 -> Draw();
  gPad -> Modified();
  gPad-> Update();

  canvas_oned_resid -> SaveAs("opticsAnalysis/opticsMatrix/ResidualPlots/phi/1d_residuals_ph_"+trgt+"_"+pass+"_"+hole_id+".pdf");
