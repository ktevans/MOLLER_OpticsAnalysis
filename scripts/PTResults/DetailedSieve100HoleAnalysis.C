...
void DetailedSieve100HoleAnalysis()
{
...
//---Define Histograms---
  ...
  TH1F *radial_clean_pass = new TH1F("radial_clean_pass","Charged Particles that Pass Through Any of the Sieve (100mm) Holes and Hit the Main Detector", 1000, 650, 1200);
  ...
  TH1F *radial_hit_shower = new TH1F("radial_hit_shower","Charged Particles that Result from Primaries Hitting the Sieve Surface and then Showering into a Sieve (100mm) Hole and Hit the Main Detector", 1000, 650, 1200);
...
//---Draw and Save Histograms---
  ...
  cleanpass -> SaveAs("images/PTResults/CP_Hole_Analysis_Sieve_100.png");
...
//---Separate the Histograms---
  ...
  auto titleCleanPassSep = new TPaveLabel(0.0, 0.9, 1.0, 1.0, "Charged Particles that Pass Through Any of the Sieve (100mm) Holes and Hit the Main Detector", "brNDC");
  titleCleanPassSep -> Draw();

  gPad -> Update();

  TPaveStats *st1sep = (TPaveStats*)radial_clean_pass -> GetListOfFunctions() -> FindObject("stats");
  st1sep -> SetY1NDC(0.9);
  st1sep -> SetY2NDC(0.75);

  separate -> cd(2);
  radial_hole_shower -> Draw();

  auto titleHoleShowerSep = new TPaveLabel(0.0, 0.9, 1.0, 1.0, "Charged Particles that Result from Primaries Hitting a Sieve Hole and then Showering Through Sieve (100mm) and Hit the Main Detector", "brNDC");
  titleHoleShowerSep -> Draw();

  gPad -> Update();

  TPaveStats *st2sep = (TPaveStats*)radial_hole_shower -> GetListOfFunctions() -> FindObject("stats");
  st2sep -> SetY1NDC(0.9);
  st2sep -> SetY2NDC(0.75);

  separate -> cd(3);
  radial_hit_shower -> Draw();

  auto titleHitShowerSep = new TPaveLabel(0.0, 0.9, 1.0, 1.0, "Charged Particles that Result from Primaries Hitting the Sieve Surface and then Showering into a Sieve (100mm) Hole and Hit the Main Detector", "brNDC");
  titleHitShowerSep -> Draw();

  gPad -> Update();

  TPaveStats *st3sep = (TPaveStats*)radial_hit_shower -> GetListOfFunctions() -> FindObject("stats");
  st3sep -> SetY1NDC(0.9);
  st3sep -> SetY2NDC(0.75);

  separate -> SaveAs("images/PTResults/CP_Hole_Analysis_Sieve_100_Separate.png");
